...
- [ ] Cross-origin: kalau frontend beda origin dari endpoint, set `NEXT_PUBLIC_UPLOAD_URL` ke URL absolut endpoint. `OPTIONS /upload` → `204` + `Access-Control-Allow-Origin: *`; POST cross-origin mengembalikan header CORS dan URL hasil dijadikan absolut oleh frontend.
